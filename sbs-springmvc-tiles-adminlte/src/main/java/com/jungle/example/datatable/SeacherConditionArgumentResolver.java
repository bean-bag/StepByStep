...
package com.jungle.example.datatable;
/**
 * Description:   <br/>
    ...
    public SearchCondition resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
        ...
        for (String name : parameterNames) {
             ...
             */
                ...
                String secendName = name.substring(getCharPosition(name, "\\[", 2)+1, getCharPosition(name, "\\]", 2));
                try {
                    if(condition.getOrder().get(index)==null) condition.getOrder().add(new Order());
                }catch(IndexOutOfBoundsException e){
                    condition.getOrder().add(new Order());
                }
                switch (secendName) {
                case "column":
                    condition.getOrder().get(index).setColumn(Integer.parseInt(webRequest.getParameter(name)));
                    break;
                case "dir":
                    condition.getOrder().get(index).setDir(webRequest.getParameter(name));
                    break;
                }
            }
